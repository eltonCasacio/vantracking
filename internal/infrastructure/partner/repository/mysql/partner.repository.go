...
func (r *partnerRepository) Create(partner *e.Partner) error {
	stmt, err := r.db.Prepare("INSERT INTO partners (id, name, description, price, phone_number, uf, city, street, number, cep, complement) values(?,?,?,?,?,?,?,?,?,?,?)")
	if err != nil {
		return err
	}
	defer stmt.Close()
...
func (r *partnerRepository) Update(partner *e.Partner) error {
	stmt, err := r.db.Prepare("UPDATE partners SET name=?, description=?, price=?, phone_number=?,  uf=?, city=?, street=?, number=?, cep=?, complement=? WHERE id = ?")
	if err != nil {
		return err
	}
	defer stmt.Close()
...
func (r *partnerRepository) FindAll() ([]e.Partner, error) {
	rows, err := r.db.Query("SELECT id, name, description, price, phone_number,  uf, city, street, number, cep, complement FROM partners")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *partnerRepository) FindByID(id string) (*e.Partner, error) {
	stmt, err := r.db.Prepare("SELECT id, name, description, price, phone_number,  uf, city, street, number, cep, complement FROM Partners WHERE id = ?")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
